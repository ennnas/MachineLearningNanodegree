...
# Add 'datatype' column that indicates if the record is original wiki answer as 0, training data 1,
# test data 2, onto the dataframe - uses stratified random sampling (with seed) to sample by task
# & plagiarism amount
...
def create_datatype(
    ...
    operator_of_compare: operator,
    value_of_compare: int,
    sampling_number: int,
    sampling_seed: int,
) -> None:
...
def train_test_dataframe(clean_df: pd.DataFrame, random_seed: int = 100) -> pd.DataFrame:
    ...
    # Initialize datatype as 0 initially for all records
    # after function 0 will remain only for original wiki answers
